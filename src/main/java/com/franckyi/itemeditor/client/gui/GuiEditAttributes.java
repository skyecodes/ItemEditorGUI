...
public class GuiEditAttributes extends GuiUpdaterScreen {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		for (int i = 0; i < itemAttributes.length; i++) {
			...
			drawString(fontRendererObj, AttributeHelper.getAttributeFromID(i).getText() + " :",
					this.width / 2 - this.width / 3 - 20, this.height / 2 - 60 + i * 25, 0xffffff);
		}
	}
	...
	public void initGui() {
		...
		for (int i = 0; i < 5; i++) {
			...
			buttonList.add(operations[i] = new GuiEnumButton(i + 10, this.width / 2, this.height / 2 - 66 + (i * 25),
					100, 20, "", EnumAttributeOperation.class));
			buttonList.add(slots[i] = new GuiEnumButton(i + 10, this.width / 2 + 110, this.height / 2 - 66 + (i * 25),
					70, 20, "", EnumAttributeSlot.class));
