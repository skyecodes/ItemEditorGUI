...
public class GuiItemEditorLore extends GuiScreen{
	...
	public void initGui() {
		...
		for(int i = 0; i < loreNumber; i++){
			...
			String lore = mc.player.getHeldItemMainhand().getOrCreateSubCompound("display").getTagList("Lore", 8).getStringTagAt(i);
		...
		buttonList.add(doneButton = new GuiButton(loreNumber, this.width/2 - 100, firstHeight+25*loreNumber + 10, 90, 20, "Done"));
		buttonList
				.add(cancelButton = new GuiButton(loreNumber+1, this.width/2 + 10, firstHeight+25*loreNumber + 10, 90, 20, "Cancel"));
	...
	private void updateClient() {
		...
		for(int i = 0; i < loresString.size(); i++){
    		mc.player.getHeldItemMainhand().getOrCreateSubCompound("display").getTagList("Lore", 8).appendTag(new NBTTagString(loresString.get(i)));
		}
	}
